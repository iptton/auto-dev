...
class LLMProvider2Test {
    ...
    fun shouldWorkWithJson() =
        runTest {
            ...
            val response = llmProvider2.request(Message("User", "hi!"), false, coroutineScope = backgroundScope)
    ...
    fun shouldFailIfResponseOnlyWithIllegalJsonStream() {
        runTest {
            ...
            val response = llmProvider2.request(Message("User", "hi!"), true, coroutineScope = backgroundScope)
            response.collectLatest { }
        }
    }

    @Test
    fun shouldIgnoreIllegalResponse() =
        runTest {
            val mockResponse =
                MockResponse()
                    .setResponseCode(200)
    ...
    fun shouldEmitChanges() =
        runTest {
            ...
            mockWebServer.enqueue(mockResponse)
            llmProvider2.request(Message("User", "hi!"), true, coroutineScope = backgroundScope).collectIndexed { index, value ->
                when (index) {
    ...
    @Test(timeout = 10000)
    fun ollamaTestStream() =
        runTest {
            val result =
                LLMProvider2
                    .Ollama(
                        "codellama:13b",
                    ).request(Message("User", "hi!"), stream = true, coroutineScope = backgroundScope)
            result.collect {
                println(it.chatMessage.content)
            }
        }
        ...
        runTest {
            val result =
                LLMProvider2
                    ...
                    ).request(Message("User", "hi!"), stream = false, coroutineScope = backgroundScope)
            result.collect {
                println(it.chatMessage.content)
            }
        }
